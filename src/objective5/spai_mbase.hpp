...
void computeRAndMk(const Matrix&A, const int Size, const Eigen::MatrixXd &Q, 
    ...
    *m_k_triangular_ptr = R.inverse() * c_triangular_ptr->segment(0,n2-1);
    ...
    if (flag==1) {
        ...
        std::unique_ptr<Eigen::Map<Eigen::VectorXd>> intermediate_eigenVector_ptr =
            std::make_unique<Eigen::Map<Eigen::VectorXd>>(intermediate.data(), intermediate.size());
        r = *intermediate_eigenVector_ptr - e_k;
    } 
